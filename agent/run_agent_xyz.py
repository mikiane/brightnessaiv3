#################   Agent X   #################
"""
Agent CLI de veille thématique via Feedly
- Usage: python run_agent_xyz.py --topic "..." --feed "..." --n_links 4 --email "..." --name "..." [--podcast no] [--html yes]
- Étapes: collecte RSS → extraction liens → résumés LLM → chronique → (option) podcast → e-mail
"""
...
from libs import lib__search_sources
import sys
import ast
...
def get_feedly_feeds(topic):
    try:
        ...
        logger.error(f"Erreur lors de l'appel de 'get_feedly_feeds' : {e}")
        raise RuntimeError("Feedly fetch failed")

    if not array_as_string:  # c'est-à-dire, si array_as_string est vide
        logger.error("Erreur : 'get_feedly_feeds' a renvoyé une chaîne vide")
        raise RuntimeError("Empty Feedly result")

    try:
        array_as_string = repr(array_as_string)
        rss_urls = ast.literal_eval(array_as_string)
        
    except Exception as e:
        logger.error(f"Erreur lors de la conversion de la chaîne en liste : {e}")
        raise RuntimeError("Feed list parsing failed")
    return(rss_urls)
...
def summarize_feeds(rss_urls, topic):
    ...
    for site_name, url, link_title in rss_urls:
        logger.info(str(datetime.now()) +  " : Summarization de " + link_title + " : " + url)
        ...
        logger.info( str(datetime.now()) +  " : RESUMÉ DE L'ARTICLE : \n\n\n\n" + "" + titre + "" + "\n\n" + summary)
...
def build_chronicle(summaries, topic):
    ...
    site = ""
    input_data = source
    chronicle = execute(prompt, site, input_data, DEFAULT_MODEL)
...
def build_large_chronicle(summaries, topic, name):
    ...
    logger.info(str(datetime.now()) +  " : SOURCES AVANT LA CREATION DES INTROS / CONCLUSIONS : \n\n " + source + "\n\n\n")
    ...
    logger.info(str(datetime.now()) +  " : INTRO : \n\n " + intro + "\n\n\n")
    ...
    logger.info(str(datetime.now()) +  " : CONCLU : \n\n " + conclu + "\n\n\n")
...
try:
    ...
    ## Execution du script
    logger.info(str(datetime.now()) +  " : Summarization des articles...\n")
    summaries = summarize_feeds(parsed_feeds, topic)
    #chronicle = build_chronicle(summaries, topic)
    
    logger.info(str(datetime.now()) +  " : Construction de la chronique...\n")
    if htmlformat:
    ...
    logger.info(str(datetime.now()) +  " : Génération des sources...\n")
    sources = generate_sources(parsed_feeds)
        
    if genpodcast:
        logger.info(str(datetime.now()) +  " : Génération du podcast...\n")
        id_podcast = generate_podcast(chronicle, topic)
        final_filename = PODCASTS_PATH + "final_podcast" + id_podcast + ".mp3"

        logger.info(str(datetime.now()) +  " : Envoi de l'email...\n")
        # output_filename = "img/" + id_podcast + ".png"
        send_email(chronicle, final_filename, topic, sources, email)
        logger.info(f"Le podcast pour le sujet '{topic}' a été créé et envoyé par mail.")
